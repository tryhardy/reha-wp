...
if ( ! function_exists( 'basic_setup' ) ) :
	function basic_setup() {
		...
		register_nav_menus( array(
			...
			'bottom-top' => __( 'Нижнее меню (верх)', 'basic' ),
			'bottom-bottom' => __( 'Нижнее меню (низ)', 'basic' )
		) );
...
if (function_exists('register_sidebar')){
	register_sidebar( array(
		 'name'          => 'Область в футере (правая колонка)', //название виджета в админ-панели
		 'id'            => 'footer-include', //идентификатор виджета
		 ...
		 'before_title'  => '<h4 class="footer-block__title mt-25">', //открывающий тег заголовка виджета
		 'after_title'   => '</h4>',//закрывающий тег заголовка виджета
		 ) );
}

if (function_exists('register_sidebar')){
	register_sidebar( array(
		 'name'          => 'Область в футере (левая колонка)', //название виджета в админ-панели
		 'id'            => 'footer-include-left', //идентификатор виджета
		 'description'   => 'виден во всех разделах сайта', //описание виджета в админ-панели
		 'before_widget' => '', //открывающий тег виджета с динамичным идентификатором
		 'after_widget'  => '', //закрывающий тег виджета с очищающим блоком
		 'before_title'  => '<h4 class="footer-block__title mt-25">', //открывающий тег заголовка виджета
...
if (function_exists('register_sidebar')){
	register_sidebar( array(
		 'name'          => 'Область в футере (левая колонка 2)', //название виджета в админ-панели
		 'id'            => 'footer-include-form', //идентификатор виджета
		 'description'   => 'виден во всех разделах сайта', //описание виджета в админ-панели
		 'before_widget' => '', //открывающий тег виджета с динамичным идентификатором
		 'after_widget'  => '', //закрывающий тег виджета с очищающим блоком
		 'before_title'  => '<h4 class="footer-block__title mt-25">', //открывающий тег заголовка виджета
...
function filter_wp_menu_args( $args ) {
	if ( $args['theme_location'] === 'bottom-top' || $args['theme_location'] === 'bottom-bottom' ) {
		$args['container']  = false;
		$args['items_wrap'] = '<ul class="%2$s">%3$s</ul>';
		$args['menu_class'] = 'footer-block__list';
	}
	return $args;
}
...
function filter_menu_item_css_id( $menu_id, $item, $args, $depth ) {
	return $args->theme_location === 'bottom-top' || $args->theme_location === 'bottom-bottom' ? '' : $menu_id;
}
...
function filter_nav_menu_css_classes( $classes, $item, $args, $depth ) {
	if ( $args->theme_location === 'bottom-top' || $args->theme_location === 'bottom-bottom' ) {
		$classes = [
			'footer-block__list_item'
		];
		if ( $item->current ) {
			$classes[] = 'footer-block__list_item--active';
		}
	}
	return $classes;
}
...
// 	if ( $args->theme_location === 'bottom-top' ) {
// 		$classes = [
// 			'menu',
// 			'menu--dropdown',
// 			'menu--vertical'
